...
public interface ILedgerService
{
    ...
    /// such an action will always contain the total eligible starting and ending balance
    /// for the customer at the time the entry was added to the ledger.
    ///
    /// ## Decrement Deductions can occur as a result of an API call to create a ledger
    /// entry (see [Add Ledger Entry](create-ledger-entry)), or automatically as a
    /// result of incurring usage. Both ledger entries present the `decrement` entry type.
    ///
    ...
    /// result in a ledger entry, also with a starting and ending balance. In order
    /// to provide better tracing capabilities for automatic deductions, Orb always
    /// associates each automatic deduction with the `event_id` at the time of ingestion,
    /// used to pinpoint _why_ credit deduction took place and to ensure that credits
    /// are never deducted without an associated usage event.
    ///
    ...
    /// weeks from now are present for a customer, they will be used before any deductions
    /// take place from a non-expiring credit block.
    ///
    /// If there are multiple blocks with the same expiration date, Orb will deduct
    /// from the block with the *lower cost basis* first (e.g. trial credits with a
    /// \$0 cost basis before paid credits with a \$5.00 cost basis).
    ///
    ...
    /// which consists of unexpiring credits. Each of these deductions will lead to
    /// a _separate_ ledger entry, one per credit block that is deducted from. By default,
    ...
    /// to the ledger indicating this event. Note that credit expiry should always happen
    /// close to a date boundary in the customer's timezone.
    ///
    /// ## Void initiated Credit blocks can be voided via the API. The `amount` on this
    /// entry corresponds to the number of credits that were remaining in the block
    /// at time of void. `void_reason` will be populated if the void is created with
    /// a reason.
    ///
    ...
    /// the changes (i.e. balance changes or transfers). 2. A ledger entry will be
    /// added to the credits ledger for this customer, and therefore returned in the
    ...
    ///   credit blocks. 3. If `invoice_settings` is specified, an invoice will be created
    /// that reflects the cost of the credits (based on   `amount` and `per_unit_cost_basis`).
    ///
    ...
    /// a cost basis per-credit, to indicate   how much in USD a customer paid for a
    /// single credit in a block. This can later be used for revenue recognition.
    ///
    ...
    /// for determining which credit blocks to decrement from. In the case that the
    /// deduction leads to multiple ledger entries, the response from this endpoint
    ...
    /// the required parameter `expiry_date` identifies the _originating_ block, and
    /// the required parameter `target_expiry_date` identifies when the transferred
