...
public interface ICostService
{
    ...
    /// price level. 4. Gain insight into key customer health metrics, such as the percent
    ...
    /// if your plan charges for compute hours, job runs, and data syncs, then this
    /// endpoint would provide a daily breakdown of your customer's cost for each of
    /// those axes.
    ///
    ...
    /// accrue in-arrears charges in a billable currency, this endpoint will return
    /// the total deduction amount, in credits, for the specified timeframe.
    ///
    ...
    /// consider costs relative to the start of the subscription's billing period. As
    /// a result, by default this endpoint returns cumulative totals since the beginning
    /// of the billing period. In particular, the `timeframe_start` of a returned timeframe
    ...
    /// in the request. If a subscription starts or ends within the timeframe, the
    /// response will only include windows where the subscription is active. If a subscription
    ...
    /// contain cumulative totals from multiple billing periods.
    ///
