...
public interface IInvoiceLineItemService
{
    ...
    /// the given name is searched for in the account.   If found, that item is used. If
    /// not found, a new item is created with that name.   The new item's name is used
    /// for the line item. - If both `item_id` and `name` are provided: The item is
    /// looked up by ID for association,   but the provided `name` is used for the line
    /// item (not the item's name).</para>
    /// </summary>
...
public interface IInvoiceLineItemServiceWithRawResponse
{
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /invoice_line_items</c>, but is otherwise the
