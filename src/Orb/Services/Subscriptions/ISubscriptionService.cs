...
public interface ISubscriptionService
{
    ...
    /// automatically for each billing cycle at the cadence that's configured in
    /// the plan definition.
    ///
    /// The default configuration for subscriptions in Orb is **In-advance billing**
    ...
    /// their `start_date`. For example, a subscription that begins on January 15th
    /// will have a billing cycle from January 15th to February 15th. Every subsequent
    ...
    /// This is equivalent to creating a price interval with the [add/edit price
    /// intervals endpoint](/api-reference/price-interval/add-or-edit-price-intervals).
    ...
    /// an existing add-on price with a `price_id` or `external_price_id` field,
    /// or by creating a new add-on price by including an object with the key `price`,
    /// identical to what would be used in the request body for the [create price
    /// endpoint](/api-reference/price/create-price). See the [Price resource](/product-catalog/price-configuration)
    /// for the specification of different price model configurations possible in
    /// this object.
    ...
    /// update only the quantity for the price, similar to the [Update price quantity](/api-reference/subscription/update-price-quantity) endpoint.
    ///
    ...
    ///
    /// If the plan has phases, each object in the list must include a number with
    /// `plan_phase_order` key to indicate which phase the adjustment should be added to.
    ///
    /// An object in the list can specify an optional `start_date` and optional `end_date`.
    /// If unspecified, the start or end date of the phase or subscription will be used.
    ///
    ...
    /// key, and it must specify an adjustment to replace it with by including an
    /// object with the key `adjustment`, identical to the adjustment object in the
    /// [add/edit price intervals endpoint](/api-reference/price-interval/add-or-edit-price-intervals).
    ///
    /// The replacement adjustment will have the same phase, if applicable, and the
    /// same start and end dates as the adjustment it replaces.
    ///
    ...
    /// a price can not be overridden.
    ///
    /// ### Maximums and Minimums Minimums and maximums, much like price overrides,
    ...
    /// is provided, then the default price maximum or minimum is used.
    ///
    ...
    ///
    /// Removing an existing minimum example ```json {   ...   "id": "price_id",
    ///  "model_type": "unit",   "unit_config": {     "unit_amount": "0.50"   },
    ///   "minimum_amount": null   ... } ```
    ///
    /// ### Discounts Discounts, like price overrides, can be useful when a new customer
    /// has negotiated a new or different discount than the default for a price. If
    /// a discount exists for a price and a null discount is provided on creation,
    /// then there will be no discount on the new subscription.
    ///
    ...
    /// is provided.
    ///
    /// Price discount example ```json {   ...   "id": "price_id",   "model_type":
    ...
    /// </summary>
    Task<MutatedSubscription> Create(SubscriptionCreateParams @params);
    ...
    /// </summary>
    Task<Subscription> Update(SubscriptionUpdateParams @params);
    ...
    /// </summary>
    Task<Subscriptions> List(SubscriptionListParams @params);

    /// <summary>
    /// This endpoint can be used to cancel an existing subscription. It returns the
    /// serialized subscription object with an `end_date` parameter that signifies
    ...
    /// will keep the subscription active until the end       of the quarter and potentially
    /// issue an invoice for any usage charges incurred in the intervening period.
    ///     - Issuing this cancellation request for a yearly subscription will keep
    /// the subscription active for the full       year. For example, a yearly subscription
    /// starting on 2021-11-01 and cancelled on 2021-12-08 will remain active
    ///   until 2022-11-01 and potentially issue charges in the intervening months
    /// for any recurring monthly usage       charges in its plan.     - **Note**:
    /// If a subscription's plan contains prices with difference cadences, the end
    /// of term date will be       determined by the largest cadence value. For example,
    /// cancelling end of term for a subscription with a       quarterly fixed fee
    /// with a monthly usage fee will result in the subscription ending at the end
    /// of the quarter.
    ///
    ...
    /// option will be invoiced immediately. This invoice will       include any
    /// usage fees incurred in the billing period up to the cancellation, along with
    /// any prorated       recurring fees for the billing period, if applicable.
    ///    - **Note**: If the subscription has a recurring fee that was paid in-advance,
    /// the prorated amount for the       remaining time period will be added to
    /// the [customer's balance](list-balance-transactions) upon immediate       cancellation.
    /// However, if the customer is ineligible to use the customer balance, the subscription
    /// cannot be       cancelled immediately.
    ///
    /// - `requested_date`: ends the subscription on a specified date, which requires
    /// a `cancellation_date` to be passed in.   If no timezone is provided, the
    /// customer's timezone is used.  For example, a subscription starting on January
    /// 1st   with a monthly price can be set to be cancelled on the first of any
    /// month after January 1st (e.g. March 1st, April   1st, May 1st). A subscription
    /// with multiple prices with different cadences defines the "term" to be the
    /// highest   cadence of the prices.
    ///
    /// Upcoming subscriptions are only eligible for immediate cancellation, which
    /// will set the `end_date` equal to the `start_date` upon cancellation.
    ///
    /// ## Backdated cancellations Orb allows you to cancel a subscription in the
    /// past as long as there are no paid invoices between the `requested_date` and
    /// the current time. If the cancellation is after the latest issued invoice,
    ...
    /// </summary>
    Task<MutatedSubscription> Cancel(SubscriptionCancelParams @params);

    /// <summary>
    /// This endpoint is used to fetch a [Subscription](/core-concepts##subscription)
    /// given an identifier.
    /// </summary>
    Task<Subscription> Fetch(SubscriptionFetchParams @params);
    ...
    /// (see the [subscription usage endpoint](fetch-subscription-usage) to fetch
    ...
    /// </summary>
    Task<SubscriptionFetchCostsResponse> FetchCosts(SubscriptionFetchCostsParams @params);
    ...
    /// _current billing period_ across each billable metric that participates in
    /// the subscription. Usage quantities returned are the result of evaluating the
    /// metric definition for the entirety of the customer's billing period.
    ///
    /// ### Default response shape Orb returns a `data` array with an object corresponding
    /// to each billable metric. Nested within this object is a `usage` array which
    /// has a `quantity` value and a corresponding `timeframe_start` and `timeframe_end`.
    ...
    ///
    /// Orb will include _every_ window in the response starting from the beginning
    ...
    /// usage on a custom timeframe.
    ///
    /// ## Custom timeframe In order to view usage for a custom timeframe rather
    ...
    /// (inclusive) and timeframe_end (exclusive), i.e. `[timeframe_start, timeframe_end)`.
    ///
    /// Note: - These timestamps must be specified in ISO 8601 format and UTC timezone,
    /// e.g. `2022-02-01T05:00:00Z`. - Both parameters must be specified if either
    /// is specified.
    ///
    /// ## Grouping by custom attributes In order to view a single metric grouped
    ...
    /// The `group_by` key denotes the event property on which to group.
    ///
    ...
    /// `group_by` key present in your events. The `quantity` value is the result
    /// of evaluating the billable metric for events filtered to a single value of
    /// the `group_by` key.
    ///
    ...
    /// usage grouped by a single attribute at a time.
    ///
    ///  When viewing grouped usage, Orb uses pagination to limit the response size
    ...
    ///  ] } ```
    ///
    /// ## Windowed usage The `granularity` parameter can be used to _window_ the
    /// usage `quantity` value into periods. When not specified, usage is returned
    /// for the entirety of the time range.
    ///
    ...
    /// - `[2022-02-01T08:00:00, 2022-02-02T08:00:00Z)` - `[2022-02-02T08:00:00, 2022-02-03T08:00:00Z)`
    ...
    ///    },         ...     ] } ```
    ///
    /// ## Decomposable vs. non-decomposable metrics Billable metrics fall into one
    /// of two categories: decomposable and non-decomposable. A decomposable billable
    /// metric, such as a sum or a count, can be displayed and aggregated across
    /// arbitrary timescales. On the other hand, a non-decomposable metric is not
    /// meaningful when only a slice of the billing window is considered.
    ///
    /// As an example, if we have a billable metric that's defined to count unique
    /// users, displaying a graph of unique users for each day is not representative
    ...
    /// of unique users in the billing period so far, which are the _cumulative_
    /// unique users.
    ///
    /// Accordingly, this endpoint returns treats these two types of metrics differently
    /// when `group_by` is specified: - Decomposable metrics can be grouped by any
    /// event property. - Non-decomposable metrics can only be grouped by the corresponding
    /// price's invoice grouping key. If no invoice grouping key is present, the
    /// metric does not support `group_by`.
    ///
    /// ## Matrix prices When a billable metric is attached to a price that uses matrix
    /// pricing, it's important to view usage grouped by those matrix dimensions.
    /// In this case, use the query parameters `first_dimension_key`, `first_dimension_value`
    /// and `second_dimension_key`, `second_dimension_value` while filtering to a
    /// specific `billable_metric_id`.
    ///
    ...
    /// </summary>
    Task<SubscriptionUsage> FetchUsage(SubscriptionFetchUsageParams @params);
    ...
    ///
    /// Prices can be added as price intervals to a subscription by specifying them
    /// in the `add` array. A `price_id` or `external_price_id` from an add-on price
    ...
    /// to the subscription). Additionally, a new price can be specified using the
    /// `price` field — this price will be created automatically.
    ///
    /// A `start_date` must be specified for the price interval. This is the date
    ...
    /// fees. The `end_date` will default to null, resulting in a price interval that
    /// will bill on a continually recurring basis. Both of these dates can be set
    /// in the past or the future and Orb will generate or modify invoices to ensure
    /// the subscription’s invoicing behavior is correct.
    ///
    ...
    /// to), as well as the start and end dates for the adjustment interval. The start
    /// and end dates of an existing adjustment interval can be edited via the `edit_adjustments`
    /// field (just like price intervals). (To "change" the amount of a discount,
    /// minimum, or maximum, then, you'll need to end the existing interval, and
    /// create a new adjustment interval with the new amount and a start date that
    /// matches the end date of the previous interval.)
    ///
    /// ## Editing price intervals
    ///
    /// Price intervals can be adjusted by specifying edits to make in the `edit`
    ...
    ///
    /// A new `start_date` or `end_date` can be specified to change the range of
    /// the price interval, which will modify past or future invoices to ensure correctness.
    /// If either of these dates are unspecified, they will default to the existing
    /// date on the price interval. To remove a price interval entirely from a subscription,
    /// set the `end_date` to be equivalent to the `start_date`.
    ///
    /// ## Fixed fee quantity transitions The fixed fee quantity transitions for
    /// a fixed fee price interval can also be specified when adding or editing by
    /// passing an array for `fixed_fee_quantity_transitions`. A fixed fee quantity
    ...
    ///
    /// Notably, the list of fixed fee quantity transitions passed will overwrite
    ...
    /// </summary>
    Task<MutatedSubscription> PriceIntervals(SubscriptionPriceIntervalsParams @params);

    /// <summary>
    /// Redeem a coupon effective at a given time.
    /// </summary>
    Task<MutatedSubscription> RedeemCoupon(SubscriptionRedeemCouponParams @params);

    /// <summary>
    /// This endpoint can be used to change an existing subscription's plan. It returns
    /// the serialized updated subscription object.
    ///
    /// The body parameter `change_option` determines when the plan change occurrs.
    ...
    /// subscription will keep the existing plan active for       the full year.
    /// Charges incurred in the remaining period will be invoiced as normal.     -
    /// Example: The plan is billed monthly on the 1st of the month, the request
    /// is made on January 15th, so the plan will be       changed on February 1st,
    /// and invoice will be issued on February 1st for the last month of the original
    /// plan. - `immediate`: changes the plan immediately.     - Subscriptions that
    /// have their plan changed with this option will move to the new plan immediately,
    /// and be invoiced       immediately.     - This invoice will include any usage
    /// fees incurred in the billing period up to the change, along with any prorated
    ...
    ///  is chosen.     - Example: The plan is billed monthly on the 1st of the month,
    /// the request is made on January 15th, with a requested       `change_date`
    /// of February 15th, so the plan will be changed on February 15th, and invoices
    /// will be issued on February 1st       and February 15th.
    ///
    /// Note that one of `plan_id` or `external_plan_id` is required in the request
    ...
    ///
    /// An object in the list can specify an optional `start_date` and optional `end_date`.
    /// If `start_date` is unspecified, the start of the phase / plan change time
    /// will be used. If `end_date` is unspecified, it will finish at the end of the
    /// phase / have no end time.
    ///
    /// An object in the list can specify an optional `minimum_amount`, `maximum_amount`,
    /// or `discounts`. This will create adjustments which apply only to this price.
    ///
    ...
    /// a price can not be overridden.
    ///
    /// ### Maximums, and minimums Price overrides are used to update some or all
    /// prices in the target plan. Minimums and maximums, much like price overrides,
    /// can be useful when a new customer has negotiated a new or different minimum
    /// or maximum spend cap than the default for the plan. The request format for
    /// maximums and minimums is the same as those in [subscription creation](create-subscription).
    ///
    /// ## Scheduling multiple plan changes When scheduling multiple plan changes
    /// with the same date, the latest plan change on that day takes effect.
    ///
    /// ## Prorations for in-advance fees By default, Orb calculates the prorated
    ...
    /// </summary>
    Task<MutatedSubscription> SchedulePlanChange(SubscriptionSchedulePlanChangeParams @params);

    /// <summary>
    /// Manually trigger a phase, effective the given date (or the current time, if
    /// not specified).
    /// </summary>
    Task<MutatedSubscription> TriggerPhase(SubscriptionTriggerPhaseParams @params);
    ...
    /// <summary>
    /// This endpoint can be used to clear scheduled updates to the quantity for
    /// a fixed fee.
    ///
    ...
    /// this subscription, pass `change_option=upcoming_invoice` without an `effective_date` specified.
    ///
    ...
    /// the new trial end date must be on or after the subscription's start date
    /// on the current plan, and on or before the subscription end date).
    ///
    /// In order to retroactively remove a trial completely, the end date can be
    /// set to the transition date of the subscription to this plan (or, if this
    /// is the first plan for this subscription, the subscription's start date). In
    /// order to end a trial immediately, the keyword `immediate` can be provided
    ...
    /// intervals untouched. If the `shift` parameter is set to `true`, Orb will
    /// shift all subsequent price and adjustment intervals by the same amount as
    /// the trial end date shift (so, e.g., if a plan change is scheduled or an add-on
    /// price was added, that change will be pushed back by the same amount of time
    /// the trial is extended).
    /// </summary>
    Task<MutatedSubscription> UpdateTrial(SubscriptionUpdateTrialParams @params);
}
