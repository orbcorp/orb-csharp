...
public interface IEventService
{
    ...
    /// that `event_id`. Note that a single event can be amended any number of times, so
    /// the same event can be overwritten in subsequent calls to this endpoint. Only a
    /// single event with a given `event_id` will be considered the source of truth at
    /// any given time.</para>
    ///
    /// <para>This is a powerful and audit-safe mechanism to retroactively update a
    /// single event in cases where you need to: * update an event with new metadata as
    /// you iterate on your pricing model * update an event based on the result of an
    /// external API call (e.g. call to a payment gateway succeeded or failed)</para>
    ///
    ...
    /// timestamps must be sent in ISO8601 format with UTC timezone offset. * The
    /// `customer_id` or `external_customer_id` of the new event must match the
    ...
    /// within Orb. Unlike ingestion, for event amendment, we strictly enforce that the
    ...
    /// current subscription's previous billing period. * By default, no more than 100
    /// events can be amended for a single customer in a 100 day period. For higher
    /// volume   updates, consider using the [event backfill](create-backfill) endpoint.</para>
    /// </summary>
    ...
    ///
    /// <para>This is a powerful and audit-safe mechanism to retroactively deprecate a
    /// single event in cases where you need to: * no longer bill for an event that was
    /// improperly reported * no longer bill for an event based on the result of an
    /// external API call (e.g. call to a payment gateway failed and   the user should
    /// not be billed)</para>
    ///
    /// <para>If you want to only change specific properties of an event, but keep the
    /// event as part of the billing calculation, use the [Amend event](amend-event)
    /// endpoint instead.</para>
    ///
    /// <para>This API is always audit-safe. The process will still retain the
    /// deprecated event, though it will be ignored for billing calculations. For
    ...
    /// event in this endpoint, since this request is by design   idempotent. On
    /// retryable errors, you should retry the request and assume the deprecation
    /// operation has not   succeeded until receipt of a 2xx. * The event's `timestamp`
    /// must fall within the customer's current subscription's billing period, or within
    ...
    /// already invoiced customers. * The `customer_id` or the `external_customer_id` of
    /// the original event ingestion request must identify a Customer   resource within
    /// Orb, even if this event was ingested during the initial integration period. We
    /// do not allow   deprecating events for customers not in the Orb system. * By
    /// default, no more than 100 events can be deprecated for a single customer in a
    /// 100 day period. For higher volume   updates, consider using the [event
    /// backfill](create-backfill) endpoint.</para>
    /// </summary>
    ...
    ///
    /// <para>1. **Data fidelity**: The accuracy of your billing model depends on a
    /// robust foundation of events. Orb's API protocol encourages usage patterns that
    /// ensure that your data is consistently complete and correct. 2. **Fast
    ...
    /// attribute usage to a given Customer. Exactly one of these   // should be
    /// specified in a given ingestion event.</para>
    ///
    ...
    ///
    /// <para>  // A string name identifying the event, usually a usage   // action. By
    /// convention, this should not contain any whitespace.   event_name: string,</para>
    ///
    /// <para>  // An ISO 8601 format date with no timezone offset.   // This should
    /// represent the time that usage occurred   // and is important to attribute usage
    /// to a given   // billing period. See the notes below on determining the
    /// timestamp.   // e.g. 2020-12-09T16:09:53Z   timestamp: string,</para>
    ///
    /// <para>  // A unique value, generated by the client, that is   // used to
    /// de-duplicate events.   // Exactly one event with a given   // idempotency key
    /// will be ingested, which allows for   // safe request retries.   idempotency_key:
    /// string</para>
    ///
    ...
    /// and   // properties can be added at any time.   properties: {     [key:
    /// string]?: string | number | boolean,   }, } ```</para>
    ///
    ...
    ///
    /// <para>- We recommend that `idempotency_key` are unique strings that you
    /// generated with V4 UUIDs, but only require that they uniquely identify an event
    /// (i.e. don’t collide). - The `timestamp` field in the event body will be used to
    /// determine which billable period a given event falls into. For example, with a
    /// monthly billing cycle starting from the first of December, Orb will calculate
    ...
    /// their usage. - If you are using matrix pricing and matching a matrix price key
    /// with a property, you should ensure the value for that property is sent as a
    /// string.</para>
    ///
    /// <para>We encourage logging this metadata with an eye towards future use cases to
    ...
    /// real time as it is occurring, timestamp should correspond to the time that usage
    /// occurred.</para>
    ///
    ...
    /// to the half-hour mark will ensure that the usage is counted within the correct
    /// period.</para>
    ///
    ...
    /// quickly as possible, and the API provides informative error messages to help you
    /// fix problems quickly.</para>
    ///
    ...
    ///
    /// <para>- Successful responses return a 200 HTTP status code. The response
    /// contains information about previously processed events. - Requests that return a
    /// `4xx` HTTP status code indicate a payload error and contain at least one event
    /// with a validation failure. An event with a validation failure can be re-sent to
    /// the ingestion endpoint (after the payload is fixed) with the original
    /// idempotency key since that key is not marked as processed. - Requests that
    /// return a `5xx` HTTP status code indicate a server-side failure. These requests
    /// should be retried in their entirety.</para>
    ///
    ...
    /// later deemed unnecessary or filtered out, we encourage you to log them to Orb if
    /// they may be relevant to billing calculations in the future.</para>
    ///
    ...
    /// allowing for a more flexible search syntax.</para>
    ///
    ...
    /// filter by. Note that an event's ID is the `idempotency_key` that   was
    /// originally used for ingestion.</para>
    ///
...
public interface IEventServiceWithRawResponse
{
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>put /events/{event_id}</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>put /events/{event_id}/deprecate</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /ingest</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /events/search</c>, but is otherwise the
