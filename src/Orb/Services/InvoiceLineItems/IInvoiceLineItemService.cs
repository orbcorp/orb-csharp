...
public interface IInvoiceLineItemService
{
    ...
    /// without `name`: The item is looked up by ID, and the item's name   is used
    /// for the line item. - If `name` is provided without `item_id`: An item with
    /// the given name is searched for in the account.   If found, that item is used.
    /// If not found, a new item is created with that name.   The new item's name
    /// is used for the line item. - If both `item_id` and `name` are provided: The
