...
public interface IEventService
{
    ...
    ///
    /// This endpoint will mark the existing event as ignored, and Orb will only use
    /// the new event passed in the body of this request as the source of truth for
    /// that `event_id`. Note that a single event can be amended any number of times,
    /// so the same event can be overwritten in subsequent calls to this endpoint.
    /// Only a single event with a given `event_id` will be considered the source of
    ...
    /// API call (e.g. call to a payment gateway succeeded or failed)
    ///
    ...
    ///   `external_customer_id` of the existing event already ingested. Exactly one
    /// of `customer_id` and   `external_customer_id` should be specified, and similar
    /// to ingestion, the ID must identify a Customer resource   within Orb. Unlike
    /// ingestion, for event amendment, we strictly enforce that the Customer must be
    /// in the Orb   system, even during the initial integration period. We do not allow
    /// updating the `Customer` an event is   associated with. * Orb does not accept
    /// an `idempotency_key` with the event in this endpoint, since this request is
    /// by design   idempotent. On retryable errors, you should retry the request and
    ...
    /// event in cases where you need to: * no longer bill for an event that was improperly
    /// reported * no longer bill for an event based on the result of an external API
    /// call (e.g. call to a payment gateway failed and   the user should not be billed)
    ///
    ...
    /// usage data.
    ///
    /// ## Request validation * Orb does not accept an `idempotency_key` with the event
    /// in this endpoint, since this request is by design   idempotent. On retryable
    /// errors, you should retry the request and assume the deprecation operation has
    /// not   succeeded until receipt of a 2xx. * The event's `timestamp` must fall
    /// within the customer's current subscription's billing period, or within the
    ///  grace period of the customer's current subscription's previous billing period.
    /// Orb does not allow deprecating   events for billing periods that have already
    /// invoiced customers. * The `customer_id` or the `external_customer_id` of the
    /// original event ingestion request must identify a Customer   resource within
    /// Orb, even if this event was ingested during the initial integration period.
    /// We do not allow   deprecating events for customers not in the Orb system. *
    /// By default, no more than 100 events can be deprecated for a single customer
    ...
    /// for your event shape, making it instant to start streaming in usage in real-time.
    ///
    /// ## Event shape
    ///
    ...
    ///
    ///   // `customer_id` is the Orb generated identifier for the Customer,   // which
    /// is returned from the Create customer API call.   customer_id: string,
    ///
    ...
    /// and is usually set to an identifier native to your system.   external_customer_id: string,
    ///
    ///   // A string name identifying the event, usually a usage   // action. By convention,
    /// this should not contain any whitespace.   event_name: string,
    ///
    ...
    /// which allows for   // safe request retries.   idempotency_key: string
    ///
    ...
    /// which billable period a given event falls into. For example, with a monthly
    /// billing cycle starting from the first of December, Orb will calculate metrics
    /// based on events that fall into the range `12-01 00:00:00 <= timestamp < 01-01 00:00:00`.
    ///
    /// ## Logging metadata
    ///
    ...
    ///
    /// - Numeric properties on events like `compute_time_ms` can later be inputs to
    /// our flexible query engine to determine usage. - Logging a region or cluster
    ...
    /// ensure full coverage for historical data. The datatype of the value in the properties
    /// dictionary is important for metric creation from an event source. Values that
    /// you wish to numerically aggregate should be of numeric type in the event.
    ///
    /// ## Determining event timestamp For cases where usage is being reported in real
    /// time as it is occurring, timestamp should correspond to the time that usage occurred.
    ///
    ...
    /// once an hour for the previous hour of usage, setting the `timestamp` to the
    /// half-hour mark will ensure that the usage is counted within the correct period.
    ///
    ...
    /// accurately map usage to the correct billing cycle and ensure that all usage
    /// is billed for in the corresponding billing period.
    ///
    ...
    /// event of network or machine failures, ensuring data fidelity. Each event in
    ...
    /// in their entirety.
    ///
    /// ## API usage and limits The ingestion API is designed made for real-time streaming
    /// ingestion and architected for high throughput. Even if events are later deemed
    /// unnecessary or filtered out, we encourage you to log them to Orb if they may
    /// be relevant to billing calculations in the future.
    ///
    ...
    /// frequently. Optionally, events can also be briefly aggregated at the source,
    ...
    /// in all cases.
    ///
    /// We strongly recommend that you only use debug mode as part of testing your initial
    /// Orb integration. Once you're ready to switch to production, disable debug mode
    /// to take advantage of improved performance and maximal throughput.
    ///
    ...
    /// following criteria: - `event_ids`: This is an explicit array of IDs to filter
    /// by. Note that an event's ID is the `idempotency_key` that   was originally
    /// used for ingestion.
    ///
