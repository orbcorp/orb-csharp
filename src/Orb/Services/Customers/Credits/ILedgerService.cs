...
public interface ILedgerService
{
    ...
    /// by multiplying the cost basis with the number of credit units added. If you
    /// invoice or handle payment of credits outside of Orb (i.e. marketplace customers),
    /// set `mark_as_paid` in the `invoice_settings` to `true` to prevent duplicate
    /// invoicing effects. * if `per_unit_cost_basis` is greater than zero, an invoice
    /// will be generated and `invoice_settings` must be included * if `invoice_settings`
    /// is passed, one of either `custom_due_date` or `net_terms` is required to determine
    /// the due date</para>
    ///
