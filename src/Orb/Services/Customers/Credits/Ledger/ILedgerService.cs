...
public interface ILedgerService
{
    ...
    /// via the API ([Add Ledger Entry](create-ledger-entry)). The ledger entry for
    /// such an action will always contain the total eligible starting and ending
    /// balance for the customer at the time the entry was added to the ledger.
    ///
    /// ## Decrement Deductions can occur as a result of an API call to create a
    /// ledger entry (see [Add Ledger Entry](create-ledger-entry)), or automatically
    ...
    /// to the customer's plan configuration. An automated deduction of this type
    /// will result in a ledger entry, also with a starting and ending balance. In
    /// order to provide better tracing capabilities for automatic deductions, Orb
    /// always associates each automatic deduction with the `event_id` at the time
    /// of ingestion, used to pinpoint _why_ credit deduction took place and to ensure
    /// that credits are never deducted without an associated usage event.
    ///
    ...
    /// appropriately. As an example, if trial credits with an expiration date of
    /// 2 weeks from now are present for a customer, they will be used before any
    /// deductions take place from a non-expiring credit block.
    ///
    /// If there are multiple blocks with the same expiration date, Orb will deduct
    /// from the block with the *lower cost basis* first (e.g. trial credits with
    /// a \$0 cost basis before paid credits with a \$5.00 cost basis).
    ///
    ...
    /// which consists of unexpiring credits. Each of these deductions will lead
    /// to a _separate_ ledger entry, one per credit block that is deducted from.
    /// By default, the customer's total credit balance in Orb can be negative as
    ...
    ///
    /// ## Credits expiry When a set of credits expire on pre-set expiration date,
    /// the customer's balance automatically reflects this change and adds an entry
    /// to the ledger indicating this event. Note that credit expiry should always
    /// happen close to a date boundary in the customer's timezone.
    ///
    /// ## Void initiated Credit blocks can be voided via the API. The `amount` on
    /// this entry corresponds to the number of credits that were remaining in the
    ...
    ///
    /// ## Amendment When credits are added to a customer's balance as a result of
    ...
    ///
    /// ## Effects of adding a ledger entry 1. After calling this endpoint, [Fetch
    ...
    /// will be added to the credits ledger for this customer, and therefore returned
    /// in the   [View Credits Ledger](fetch-customer-credits-ledger) response as
    /// well as serialized in the response to this request. In   the case of deductions
    /// without a specified block, multiple ledger entries may be created if the
    /// deduction spans   credit blocks. 3. If `invoice_settings` is specified, an
    /// invoice will be created that reflects the cost of the credits (based on
    ...
    ///   to assist with auditing. When adding credits, the caller can also specify
    /// a cost basis per-credit, to indicate   how much in USD a customer paid for
    /// a single credit in a block. This can later be used for revenue recognition.
    ///
    /// The following snippet illustrates a sample request body to increment credits
    /// which will expire in January of 2022.
    ///
    /// ```json {   "entry_type": "increment",   "amount": 100,   "expiry_date":
    /// "2022-12-28",   "per_unit_cost_basis": "0.20",   "description": "Purchased
    /// 100 credits" } ```
    ///
    /// Note that by default, Orb will always first increment any _negative_ balance
    /// in existing blocks before adding the remaining amount to the desired credit block.
    ///
    /// ### Invoicing for credits By default, Orb manipulates the credit ledger but
    /// does not charge for credits. However, if you pass `invoice_settings` in the
    ...
    /// since the total charges on the invoice are calculated by multiplying the
    /// cost basis with the number of credit units added.
    ///
    /// ## Deducting Credits Orb allows you to deduct credits from a customer by
    ...
    /// endpoint will be the final deduction. Orb also optionally allows specifying
    /// a description to assist with auditing.
    ///
    ...
    /// the required parameter `expiry_date` identifies the _originating_ block,
    /// and the required parameter `target_expiry_date` identifies when the transferred
    /// credits should now expire. A new credit block will be created with expiry
    /// date `target_expiry_date`, with the same cost basis data as the original credit
    /// block, if present.
    ///
    /// Note that the balance of the block with the given `expiry_date` must be at
    /// least equal to the desired transfer amount determined by the `amount` parameter.
    ///
    ...
    /// For this entry, `block_id` is required to identify the block, and `amount`
