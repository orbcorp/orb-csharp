...
public interface ISubscriptionService
{
    ...
    ///
    /// <para>The default configuration for subscriptions in Orb is **In-advance
    /// billing** and **Beginning of month alignment** (see
    /// [Subscription](/core-concepts##subscription) for more details).</para>
    ///
    ...
    /// invoice on the 15th.</para>
    ///
    /// <para>If the "day" value is greater than the number of days in the month, the
    /// next billing cycle will start at the end of the month. For example, if the
    /// start_date is January 31st, the next billing cycle will start on February 28th.</para>
    ///
    /// <para>If a customer was created with a currency, Orb only allows subscribing the
    ...
    ///
    /// <para>To add prices, provide a list of objects with the key `add_prices`. An
    /// object in the list must specify an existing add-on price with a `price_id` or
    ...
    /// the [create price endpoint](/api-reference/price/create-price). See the [Price
    /// resource](/product-catalog/price-configuration) for the specification of
    /// different price model configurations possible in this object.</para>
    ///
    ...
    /// price intervals
    /// endpoint](/api-reference/price-interval/add-or-edit-price-intervals). If
    /// unspecified, the start or end date of the phase or subscription will be used.</para>
    ///
    /// <para>An object in the list can specify an optional `minimum_amount`,
    /// `maximum_amount`, or `discounts`. This will create adjustments which apply only
    /// to this price.</para>
    ///
    /// <para>Additionally, an object in the list can specify an optional
    /// `reference_id`. This ID can be used to reference this price when [adding an
    /// adjustment](#adding-adjustments) in the same API call. However the ID is
    /// _transient_ and cannot be used to refer to the price in future API calls.</para>
    ///
    /// <para>### Removing Prices</para>
    ///
    /// <para>To remove prices, provide a list of objects with the key `remove_prices`.
    /// An object in the list must specify a plan price with either a `price_id` or
    /// `external_price_id` field.</para>
    ///
    /// <para>### Replacing Prices</para>
    ///
    /// <para>To replace prices, provide a list of objects with the key
    /// `replace_prices`. An object in the list must specify a plan price to replace
    /// with the `replaces_price_id` key, and it must specify a price to replace it with
    /// by either referencing an existing add-on price with a `price_id` or
    ...
    /// only the quantity for the price, similar to the [Update price
    /// quantity](/api-reference/subscription/update-price-quantity) endpoint.</para>
    ///
    ...
    ///
    /// <para>Additionally, an object in the list can specify an optional
    /// `reference_id`. This ID can be used to reference the replacement price when
    /// [adding an adjustment](#adding-adjustments) in the same API call. However the ID
    /// is _transient_ and cannot be used to refer to the price in future API calls.</para>
    ///
    /// <para>### Adding adjustments</para>
    ///
    /// <para>To add adjustments, provide a list of objects with the key
    /// `add_adjustments`. An object in the list must include an object with the key
    /// `adjustment`, identical to the adjustment object in the [add/edit price
    /// intervals endpoint](/api-reference/price-interval/add-or-edit-price-intervals).</para>
    ///
    /// <para>If the plan has phases, each object in the list must include a number with
    ...
    ///
    /// <para>To remove adjustments, provide a list of objects with the key
    /// `remove_adjustments`. An object in the list must include a key, `adjustment_id`,
    /// with the ID of the adjustment to be removed.</para>
    ///
    /// <para>### Replacing adjustments</para>
    ///
    /// <para>To replace adjustments, provide a list of objects with the key
    ...
    /// to the adjustment object in the [add/edit price intervals
    /// endpoint](/api-reference/price-interval/add-or-edit-price-intervals).</para>
    ///
    ...
    ///
    /// <para><Note> Price overrides are being phased out in favor
    /// adding/removing/replacing prices. (See [Customize your customer's
    /// subscriptions](/api-reference/subscription/create-subscription)) </Note></para>
    ///
    /// <para>Price overrides are used to update some or all prices in a plan for the
    /// specific subscription being created. This is useful when a new customer has
    /// negotiated a rate that is unique to the customer.</para>
    ///
    /// <para>To override prices, provide a list of objects with the key
    /// `price_overrides`. The price object in the list of overrides is expected to
    /// contain the existing price id, the `model_type` and configuration. (See the
    /// [Price resource](/product-catalog/price-configuration) for the specification of
    /// different price model configurations.) The numerical values can be updated, but
    /// the billable metric, cadence, type, and name of a price can not be overridden.</para>
    ///
    /// <para>### Maximums and Minimums Minimums and maximums, much like price
    /// overrides, can be useful when a new customer has negotiated a new or different
    /// minimum or maximum spend cap than the default for a given price. If one exists
    /// for a price and null is provided for the minimum/maximum override on creation,
    /// then there will be no minimum/maximum on the new subscription. If no value is
    /// provided, then the default price maximum or minimum is used.</para>
    ///
    ...
    ///
    /// <para>```json {   ...   "id": "price_id",   "model_type": "unit",
    /// "unit_config": {     "unit_amount": "0.50"   },   "minimum_amount": "100.00"
    /// ... } ```</para>
    ///
    ...
    /// customer has negotiated a new or different discount than the default for a
    /// price. If a discount exists for a price and a null discount is provided on
    /// creation, then there will be no discount on the new subscription.</para>
    ///
    /// <para>To add a discount for a specific price, add `discount` to the price in the
    /// `price_overrides` object. Discount should be a dictionary of the format: ```ts {
    ///   "discount_type": "amount" | "percentage" | "usage",   "amount_discount":
    /// string,   "percentage_discount": string,   "usage_discount": string } ``` where
    /// either `amount_discount`, `percentage_discount`, or `usage_discount` is
    /// provided.</para>
    ///
    /// <para>Price discount example ```json {   ...   "id": "price_id",   "model_type":
    /// "unit",   "unit_config": {     "unit_amount": "0.50"   },   "discount":
    /// {"discount_type": "amount", "amount_discount": "175"}, } ```</para>
    ///
    /// <para>Removing an existing discount example ```json {   "customer_id":
    /// "customer_id",   "plan_id": "plan_id",   "discount": null,   "price_overrides":
    /// [ ... ]   ... } ```</para>
    ///
    ...
    /// which is specified in the subscription's invoicing currency, when creating a
    /// subscription. E.g. pass in `10.00` to issue an invoice when usage amounts hit
    /// \$10.00 for a subscription that invoices in USD.</para>
    ///
    ...
    /// Orb supports three cancellation options: - `end_of_subscription_term`: stops the
    ...
    /// subscription active until the start       of the subsequent month, and
    ...
    /// potentially issue an invoice for any usage charges incurred in the intervening
    /// period.     - Issuing this cancellation request for a yearly subscription will
    /// keep the subscription active for the full       year. For example, a yearly
    /// subscription starting on 2021-11-01 and cancelled on 2021-12-08 will remain
    /// active       until 2022-11-01 and potentially issue charges in the intervening
    /// months for any recurring monthly usage       charges in its plan.     -
    /// **Note**: If a subscription's plan contains prices with difference cadences, the
    /// end of term date will be       determined by the largest cadence value. For
    /// example, cancelling end of term for a subscription with a       quarterly fixed
    /// fee with a monthly usage fee will result in the subscription ending at the end
    /// of the quarter.</para>
    ///
    ...
    /// [customer's balance](list-balance-transactions) upon immediate
    /// cancellation. However, if the customer is ineligible to use the customer
    /// balance, the subscription cannot be       cancelled immediately.</para>
    ///
    ...
    /// the customer's timezone is used.  For example, a subscription starting on January
    /// 1st   with a monthly price can be set to be cancelled on the first of any month after
    /// January 1st (e.g. March 1st, April   1st, May 1st). A subscription with multiple
    /// prices with different cadences defines the "term" to be the highest   cadence of
    /// the prices.</para>
    ///
    ...
    /// the most recently issued invoice, Orb will void the intervening invoice and
    /// generate a new one based on the new dates for the subscription. See the section
    /// on [cancellation
    /// behaviors](/product-catalog/creating-subscriptions#cancellation-behaviors).</para>
    /// </summary>
    ...
    /// <inheritdoc cref="Fetch(SubscriptionFetchParams, CancellationToken)"/>
    Task<Subscription> Fetch(
        string subscriptionID,
    ...
    /// <inheritdoc cref="FetchCosts(SubscriptionFetchCostsParams, CancellationToken)"/>
    Task<SubscriptionFetchCostsResponse> FetchCosts(
        string subscriptionID,
    ...
    /// array which has a `quantity` value and a corresponding `timeframe_start` and
    /// `timeframe_end`.  The `quantity` value represents the calculated usage value for
    /// the billable metric over the specified timeframe (inclusive of the `timeframe_start`
    /// timestamp and exclusive of the `timeframe_end` timestamp).</para>
    ///
    /// <para>Orb will include _every_ window in the response starting from the
    /// beginning of the billing period, even when there were no events (and therefore
    /// no usage) in the window. This increases the size of the response but prevents
    /// the caller from filling in gaps and handling cumbersome time-based logic.</para>
    ///
    ...
    /// than the current billing period, specify a `timeframe_start` and
    /// `timeframe_end`. This will calculate quantities for usage incurred between
    /// timeframe_start (inclusive) and timeframe_end (exclusive), i.e.
    /// `[timeframe_start, timeframe_end)`.</para>
    ///
    /// <para>Note: - These timestamps must be specified in ISO 8601 format and UTC
    /// timezone, e.g. `2022-02-01T05:00:00Z`. - Both parameters must be specified if
    /// either is specified.</para>
    ///
    ...
    /// `group_by` key denotes the event property on which to group.</para>
    ///
    ...
    /// By default, Orb will not return a `null` group (i.e. events that match the
    /// metric but do not have the key set). Currently, it is only possible to view
    /// usage grouped by a single attribute at a time.</para>
    ///
    ...
    /// "2021-10-01T07:00:00Z",                     "timeframe_end":
    /// "2021-10-02T07:00:00Z",                 },                 ...             ],
    ///           "metric_group": {                 "property_key": "region",
    ///       "property_value": "asia/pacific"             },
    /// "billable_metric": {                 "id": "Fe9pbpMk86xpwdGB",
    /// "name": "API Requests"             },             "view_mode": "periodic"
    ///   },         ...     ] } ```</para>
    ///
    ...
    /// and `timeframe_end`. Note that the days are demarcated by the _customer's local
    /// midnight_.</para>
    ///
    ...
    /// returned for a customer in the `America/Los_Angeles` timezone since local
    ...
    /// "2022-02-03T08:00:00+00:00",                     "timeframe_start":
    /// "2022-02-02T08:00:00+00:00"                 },                 {
    ///     "quantity": 0,                     "timeframe_end":
    /// "2022-02-04T01:00:00+00:00",                     "timeframe_start":
    /// "2022-02-03T08:00:00+00:00"                 }             ],
    /// "view_mode": "periodic"         },         ...     ] } ```</para>
    ///
    /// <para>## Decomposable vs. non-decomposable metrics Billable metrics fall into
    /// one of two categories: decomposable and non-decomposable. A decomposable
    /// billable metric, such as a sum or a count, can be displayed and aggregated
    /// across arbitrary timescales. On the other hand, a non-decomposable metric is not
    /// meaningful when only a slice of the billing window is considered.</para>
    ///
    ...
    /// In this case, use the query parameters `first_dimension_key`,
    ...
    ///
    /// <para>For example, if your compute metric has a separate unit price (i.e. a
    /// matrix pricing model) per `region` and `provider`, your request might provide
    ...
    /// <inheritdoc cref="FetchUsage(SubscriptionFetchUsageParams, CancellationToken)"/>
    Task<SubscriptionUsage> FetchUsage(
        string subscriptionID,
    ...
    /// intervals](/api-reference/price-interval/add-or-edit-price-intervals). By making
    /// modifications to a subscription’s price intervals, you can [flexibly and
    /// atomically control the billing behavior of a
    ...
    /// price definition (however, please note that prices from other plans cannot be
    /// added to the subscription). Additionally, a new price can be specified using the
    /// `price` field — this price will be created automatically.</para>
    ///
    /// <para>A `start_date` must be specified for the price interval. This is the date
    /// when the price will start billing on the subscription, so this will notably
    /// result in an immediate charge at this time for any billed in advance fixed fees.
    /// The `end_date` will default to null, resulting in a price interval that will
    /// bill on a continually recurring basis. Both of these dates can be set in the
    /// past or the future and Orb will generate or modify invoices to ensure the
    /// subscription’s invoicing behavior is correct.</para>
    ///
    /// <para>Additionally, a discount, minimum, or maximum can be specified on the
    /// price interval. This will only apply to this price interval, not any other price
    /// intervals on the subscription.</para>
    ///
    /// <para>## Adjustment intervals</para>
    ///
    /// <para>An adjustment interval represents the time period that a particular
    /// adjustment (a discount, minimum, or maximum) applies to the prices on a
    /// subscription. Adjustment intervals can be added to a subscription by specifying
    /// them in the `add_adjustments` array, or modified via the `edit_adjustments`
    /// array. When creating an adjustment interval, you'll need to provide the
    /// definition of the new adjustment (the type of adjustment, and which prices it
    /// applies to), as well as the start and end dates for the adjustment interval. The
    /// start and end dates of an existing adjustment interval can be edited via the
    /// `edit_adjustments` field (just like price intervals). (To "change" the amount of
    /// a discount, minimum, or maximum, then, you'll need to end the existing interval,
    /// and create a new adjustment interval with the new amount and a start date that
    /// matches the end date of the previous interval.)</para>
    ///
    ...
    /// the price interval, which will modify past or future invoices to ensure
    /// correctness. If either of these dates are unspecified, they will default to the
    /// existing date on the price interval. To remove a price interval entirely from a
    /// subscription, set the `end_date` to be equivalent to the `start_date`.</para>
    ///
    /// <para>## Fixed fee quantity transitions The fixed fee quantity transitions for a
    /// fixed fee price interval can also be specified when adding or editing by passing
    ...
    /// the new quantity will be used for billing. If a fixed fee quantity transition is
    ...
    /// <inheritdoc cref="PriceIntervals(SubscriptionPriceIntervalsParams, CancellationToken)"/>
    Task<MutatedSubscription> PriceIntervals(
        string subscriptionID,
    ...
    /// <inheritdoc cref="RedeemCoupon(SubscriptionRedeemCouponParams, CancellationToken)"/>
    Task<MutatedSubscription> RedeemCoupon(
        string subscriptionID,
        SubscriptionRedeemCouponParams parameters,
        CancellationToken cancellationToken = default
    ...
    /// incurred in the remaining period will be invoiced as normal.     - Example: The
    /// plan is billed monthly on the 1st of the month, the request is made on January
    /// 15th, so the plan will be       changed on February 1st, and invoice will be
    /// issued on February 1st for the last month of the original plan. - `immediate`:
    /// changes the plan immediately.     - Subscriptions that have their plan changed
    /// with this option will move to the new plan immediately, and be invoiced
    /// immediately.     - This invoice will include any usage fees incurred in the
    /// billing period up to the change, along with any prorated       recurring fees
    /// for the billing period, if applicable.     - Example: The plan is billed monthly
    /// on the 1st of the month, the request is made on January 15th, so the plan will
    /// be       changed on January 15th, and an invoice will be issued for the partial
    /// month, from January 1 to January 15, on the       original plan. -
    /// `requested_date`: changes the plan on the requested date (`change_date`).     -
    /// If no timezone is provided, the customer's timezone is used. The `change_date`
    /// body parameter is required if this option       is chosen.     - Example: The
    /// plan is billed monthly on the 1st of the month, the request is made on January
    /// 15th, with a requested       `change_date` of February 15th, so the plan will be
    /// changed on February 15th, and invoices will be issued on February 1st       and
    /// February 15th.</para>
    ///
    ...
    /// <para><Note> Price overrides are being phased out in favor
    /// adding/removing/replacing prices. (See [Customize your customer's
    /// subscriptions](/api-reference/subscription/schedule-plan-change)) </Note></para>
    ///
    /// <para>Price overrides are used to update some or all prices in a plan for the
    /// specific subscription being created. This is useful when a new customer has
    /// negotiated a rate that is unique to the customer.</para>
    ...
    /// be useful when a new customer has negotiated a new or different minimum or
    /// maximum spend cap than the default for the plan. The request format for maximums
    /// and minimums is the same as those in [subscription
    /// creation](create-subscription).</para>
    ///
    ...
    /// balance as needed. For details on this behavior, see [Modifying
    /// subscriptions](/product-catalog/modifying-subscriptions#prorations-for-in-advance-fees).</para>
    /// </summary>
    ...
    /// <inheritdoc cref="SchedulePlanChange(SubscriptionSchedulePlanChangeParams, CancellationToken)"/>
    Task<MutatedSubscription> SchedulePlanChange(
        string subscriptionID,
        SubscriptionSchedulePlanChangeParams parameters,
        CancellationToken cancellationToken = default
    ...
    /// <inheritdoc cref="TriggerPhase(SubscriptionTriggerPhaseParams, CancellationToken)"/>
    Task<MutatedSubscription> TriggerPhase(
        string subscriptionID,
    ...
    /// future cancellation. This operation will turn on auto-renew, ensuring that the
    /// subscription does not end at the currently scheduled cancellation time.</para>
    ///
    /// <para>Note: uncancellation is a lossy operation. Price intervals that were cut
    /// short by the cancellation are extended to infinity (original end dates are
    /// lost), and future intervals or phases scheduled after the cancellation time are
    /// permanently deleted. For complex subscriptions with phases or scheduled plan
    /// changes, consider creating a new plan change instead of uncancelling.</para>
    /// </summary>
    ...
    /// <inheritdoc cref="UnscheduleCancellation(SubscriptionUnscheduleCancellationParams, CancellationToken)"/>
    Task<MutatedSubscription> UnscheduleCancellation(
        string subscriptionID,
    ...
    /// <inheritdoc cref="UnscheduleFixedFeeQuantityUpdates(SubscriptionUnscheduleFixedFeeQuantityUpdatesParams, CancellationToken)"/>
    Task<MutatedSubscription> UnscheduleFixedFeeQuantityUpdates(
        string subscriptionID,
        SubscriptionUnscheduleFixedFeeQuantityUpdatesParams parameters,
        CancellationToken cancellationToken = default
    ...
    /// <inheritdoc cref="UnschedulePendingPlanChanges(SubscriptionUnschedulePendingPlanChangesParams, CancellationToken)"/>
    Task<MutatedSubscription> UnschedulePendingPlanChanges(
        string subscriptionID,
    ...
    /// timezone.</para>
    ///
    /// <para>In order to change the fixed fee quantity as of the next draft invoice for
    /// this subscription, pass `change_option=upcoming_invoice` without an
    /// `effective_date` specified.</para>
    ///
    ...
    /// <inheritdoc cref="UpdateFixedFeeQuantity(SubscriptionUpdateFixedFeeQuantityParams, CancellationToken)"/>
    Task<MutatedSubscription> UpdateFixedFeeQuantity(
        string subscriptionID,
        SubscriptionUpdateFixedFeeQuantityParams parameters,
        CancellationToken cancellationToken = default
    ...
    /// <inheritdoc cref="UpdateTrial(SubscriptionUpdateTrialParams, CancellationToken)"/>
    Task<MutatedSubscription> UpdateTrial(
        string subscriptionID,
        SubscriptionUpdateTrialParams parameters,
        CancellationToken cancellationToken = default
...
public interface ISubscriptionServiceWithRawResponse
{
    ...
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> Create(
        SubscriptionCreateParams? parameters = null,
        CancellationToken cancellationToken = default
    );

    /// <summary>
    /// Returns a raw HTTP response for <c>put /subscriptions/{subscription_id}</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/cancel</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.Cancel(SubscriptionCancelParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> Cancel(
        SubscriptionCancelParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// same as <see cref="ISubscriptionService.Fetch(SubscriptionFetchParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<Subscription>> Fetch(
        SubscriptionFetchParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>get /subscriptions/{subscription_id}/costs</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.FetchCosts(SubscriptionFetchCostsParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<SubscriptionFetchCostsResponse>> FetchCosts(
        SubscriptionFetchCostsParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>get /subscriptions/{subscription_id}/schedule</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.FetchSchedule(SubscriptionFetchScheduleParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<SubscriptionFetchSchedulePage>> FetchSchedule(
        SubscriptionFetchScheduleParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>get /subscriptions/{subscription_id}/usage</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.FetchUsage(SubscriptionFetchUsageParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<SubscriptionUsage>> FetchUsage(
        SubscriptionFetchUsageParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/price_intervals</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.PriceIntervals(SubscriptionPriceIntervalsParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> PriceIntervals(
        SubscriptionPriceIntervalsParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/redeem_coupon</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.RedeemCoupon(SubscriptionRedeemCouponParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> RedeemCoupon(
        SubscriptionRedeemCouponParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/schedule_plan_change</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.SchedulePlanChange(SubscriptionSchedulePlanChangeParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> SchedulePlanChange(
        SubscriptionSchedulePlanChangeParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/trigger_phase</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.TriggerPhase(SubscriptionTriggerPhaseParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> TriggerPhase(
        SubscriptionTriggerPhaseParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/unschedule_cancellation</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.UnscheduleCancellation(SubscriptionUnscheduleCancellationParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> UnscheduleCancellation(
        SubscriptionUnscheduleCancellationParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/unschedule_fixed_fee_quantity_updates</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.UnscheduleFixedFeeQuantityUpdates(SubscriptionUnscheduleFixedFeeQuantityUpdatesParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> UnscheduleFixedFeeQuantityUpdates(
        SubscriptionUnscheduleFixedFeeQuantityUpdatesParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/unschedule_pending_plan_changes</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.UnschedulePendingPlanChanges(SubscriptionUnschedulePendingPlanChangesParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> UnschedulePendingPlanChanges(
        SubscriptionUnschedulePendingPlanChangesParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/update_fixed_fee_quantity</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.UpdateFixedFeeQuantity(SubscriptionUpdateFixedFeeQuantityParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> UpdateFixedFeeQuantity(
        SubscriptionUpdateFixedFeeQuantityParams parameters,
        CancellationToken cancellationToken = default
    );
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /subscriptions/{subscription_id}/update_trial</c>, but is otherwise the
    /// same as <see cref="ISubscriptionService.UpdateTrial(SubscriptionUpdateTrialParams, CancellationToken)"/>.
    /// </summary>
    Task<HttpResponse<MutatedSubscription>> UpdateTrial(
        SubscriptionUpdateTrialParams parameters,
        CancellationToken cancellationToken = default
    );
