...
public sealed record class Data : Orb::ModelBase, Orb::IFromRaw<Data>
{
    ...
    public required DataProperties::MetricGroup MetricGroup
    {
        get
        {
            if (!this.Properties.TryGetValue("metric_group", out Json::JsonElement element))
                throw new System::ArgumentOutOfRangeException(
                    "metric_group",
                    "Missing required argument"
                );

            return Json::JsonSerializer.Deserialize<DataProperties::MetricGroup>(element)
                ?? throw new System::ArgumentNullException("metric_group");
        }
        set { this.Properties["metric_group"] = Json::JsonSerializer.SerializeToElement(value); }
    }
