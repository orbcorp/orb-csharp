...
using Orb.Models.Subscriptions.NewSubscriptionScalableMatrixWithUnitPricingPriceProperties.ScalableMatrixWithUnitPricingConfigProperties;

namespace Orb.Models.Subscriptions.NewSubscriptionScalableMatrixWithUnitPricingPriceProperties;
    ...
    public required string UnitPrice
    {
        get
        {
            if (!this.Properties.TryGetValue("unit_price", out JsonElement element))
                throw new ArgumentOutOfRangeException("unit_price", "Missing required argument");

            return JsonSerializer.Deserialize<string>(element, ModelBase.SerializerOptions)
                ?? throw new ArgumentNullException("unit_price");
        }
        set
        {
